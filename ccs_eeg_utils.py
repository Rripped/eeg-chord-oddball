...
def download_erpcore(task="MMN", subject=1, localpath="local/bids/"):
    ...
    for extension in ["channels.tsv", "events.tsv", "eeg.fdt", "eeg.json", "eeg.set"]:
        targetpath = "/sub-{:03d}/ses-{}/eeg/sub-{:03d}_ses-{}_task-{}_{}".format(
            subject, task, subject, task, task, extension
        )
...
def simulate_ICA(dims=4):
    ...
    s.append(
        (
            np.mod(np.array(range(0, nsamples)), 100)
            + np.array(range(0, nsamples)) * 0.05
        )
        / 100
    )
    if dims == 4:
        ...
        s.append(
            ...
            + 0.2 * np.sin(2 * np.pi * 15.3 * t)
            + 0.1 * np.sin(2 * np.pi * 16.7 * t + 0.1)
...
def simulate_TF(signal=1, noise=True):
    ...
    if signal == 2:
        ...
        sig = scipy.signal.chirp(
            t=np.arange(0, 10, 1 / 500), f0=1, f1=100, t1=2, method="linear", phi=90
        )
    elif signal == 1:
        x = np.arange(0, 2, 1 / 500)
        sig_steps = np.concatenate(
            [
                1.0 * sin(2 * np.pi * x * 50),
                1.2 * sin(2 * np.pi * x * 55 + np.pi / 2),
...
def ex8_simulateData(
    ...
    # Add three signals
    X[:, width // 6 * 2, width // 6 * 2] -= (
        signal_mean / 3 * 3 + rng.randn(n_subjects) * noise_between
    )
    X[:, width // 6 * 4, width // 6 * 4] += (
        signal_mean / 3 * 2 + rng.randn(n_subjects) * noise_between
    )
    X[:, width // 6 * 5, width // 6 * 5] += (
        signal_mean / 3 * 2 + rng.randn(n_subjects) * noise_between
    )
